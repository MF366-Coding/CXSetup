...
def syntax_highlight(code: str, colormap: dict[str, str] | None = None, ignore_unreachable_code: bool = True):
    ...
    for line in lines:
        ...
        tokens = re.split(r'(\s+|".*?"|[?:;]|[\?]{2}|c[0-9]+:[0-9]+[el]:[0-9]+:[0-9]+:[rlbnRLBNekvm])', line)
        semicolon = False
        comment = False
        ...
        for token in tokens:
            ...
            # [*] Always Run
            if token in {'&RUN', '&SAFECIN', '&SUM', '&SET', '&SUB', '&FILE.EXISTS', '&CLEAR', '&DIR.EXISTS', '&BACK', '&PATH.EXISTS', '&ECHO', '&STYLE', '&RESET', '&REQUIRES', '&CIN', '&YAYORNAY', '&ENDL2', '&COUT', '&REQINSTALL', '&ENDL', '&REM', '&GETPASS', '&ABS', '&INVERT', '&PKGRUN', '&ROUND', '&DIV', '&FORE', '&PIPRUN', '&NPMRUN', '&TERMINATE', '&ECHORDIE'}:
                text.append(token, colormap.get('ALWAYS_RUN', colormap.get('CACHE_GRAB')))
                semicolon = False
            ...
            # [*] Negations
            if token in {'!INVERT', '!PATH.EXISTS', '!FILE.EXISTS', '!DIR.EXISTS', '!RESET', '!SUM', '!REM', '!DIV', '!SUB', '!ABS', '!ROUND', '!PROD', '!ENDL', '!ENDL2', '!GETPASS', '!COUT', '!ECHO', '!CIN', '!TERMINATE', '!REQUIRES', '!REQINSTALL', '!STYLE', '!FORE', '!BACK', '!CLEAR', '!SET', '!ECHORDIE', '!SAFECIN', '!YAYORNAY', '!PKGRUN', '!RUN', '!PIPRUN', '!NPMRUN'}:
            ...
            # [*] Keywords/Functions
            if token in {'INVERT', 'PATH.EXISTS', 'FILE.EXISTS', 'DIR.EXISTS', 'RESET', 'SUM', 'REM', 'DIV', 'SUB', 'ABS', 'ROUND', 'ENDL', 'ENDL2', 'GETPASS', 'COUT', 'ECHO', 'CIN', 'TERMINATE', 'REQUIRES', 'REQINSTALL', 'STYLE', 'FORE', 'BACK', 'CLEAR', 'SET', 'ECHORDIE', 'SAFECIN', 'YAYORNAY', 'PKGRUN', 'RUN', 'PIPRUN', 'NPMRUN'}:
                text.append(token, colormap.get('POSITIVE', colormap.get('FUNCTION')))
                semicolon = False
            ...
            # [*] Floats
            if re.fullmatch(r'[0-9]+\.[0-9]+', token) or re.fullmatch('[0-9]+[e][0-9]+', token) or re.fullmatch(r'[-][0-9]+\.[0-9]+', token) or re.fullmatch('[-][0-9]+[e][0-9]+', token):
                text.append(token, colormap['NUMBER'])
                semicolon = False
                continue

            # [*] Integer
            if re.fullmatch(r'[0-9]+', token) or re.fullmatch(r'[-][0-9]+', token):
                text.append(token, colormap['NUMBER'])
                semicolon = False
                continue
