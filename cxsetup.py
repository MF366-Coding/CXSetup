...
class Interpreter:
    def __init__(self, script: str, cache_bytes: int, flag_marker: FlagMarker, pkg_command: str, pip_path: str = 'pip', npm_path: str = 'npm') -> None:
        ...
        self.CXVARS = {
            ###### GENERIC VARIABLES ######
            "polarity": self._specialized_cache.cache[0],

            ###### CX SETUP VARIABLES ######
            "cxSetup.Version": VERSION
        }
    ...
    def handle_cache_grab(self, declaration: str) -> bytes | bool | int | str | char | float:
        ...
        # [*] Let's use regex
        PATTERN = "c[0-9]+:[0-9]+[el]:[0-9]+:[0-9]+:[rlbnRLBNvmek]"
        ...
        match fifth_arg:
            ...
            case 'B':
                parsed_cache = parsed_cache.strip().upper()

            case 'R':
                parsed_cache = parsed_cache.rstrip().upper()

            case 'L':
                parsed_cache = parsed_cache.lstrip().upper()

            case 'N':
                parsed_cache = parsed_cache.upper()

            case 'v':
                parsed_cache = parsed_cache.strip().lower()

            case 'e':
                parsed_cache = parsed_cache.rstrip().lower()

            case 'k':
                parsed_cache = parsed_cache.lstrip().lower()
    ...
    def handle_statement(self, statement: str):
        ...
        if declaration[0] == '!':
            ...
            is_positive = is_globally_positive
            func = parts[0][1:]
        ...
        for arg in arguments:
            ...
            if parsed_arg in ('true', 'false'):
                args.append(0 if parsed_arg == 'false' else 1)
                continue

            if re.match(r'[0-9]+\.[0-9]+', parsed_arg) or re.match('[0-9]+[e][0-9]+', parsed_arg) or re.match(r'[-][0-9]+\.[0-9]+', parsed_arg) or re.match('[-][0-9]+[e][0-9]+', parsed_arg):
                # [i] data type: float
                args.append(float(parsed_arg))
                continue

            if re.match('[0-9]+', parsed_arg) or re.match('[-][0-9]+', parsed_arg):
                # [i] data type: integer
            ...
            if parsed_arg.startswith('"') and parsed_arg.endswith('"') and parsed_arg.count('"') == 2:
                ...
                args.append(parsed_arg[1:-1].replace(r"\S", ";").replace(r"\?", "?").replace(r"\2", "2").replace(r'\\', chr(92)))
                continue
            ...
            # [*] is it a CX Setup data source?
            if parsed_arg in self.CXVARS:
                args.append(self.CXVARS[parsed_arg])
                continue
        ...
        match func:
            ...
            case 'TERMINATE':
                if len(args) > 2:
                    self.raise_error(SyntaxError, f"0, 1 or 2 arguments were expected, but {len(args)} arguments were received")
                ...
                if args:
                    ...
                    if len(args) == 2:
                        if not isinstance(args[1], str):
                            self.raise_error(TypeError, 'a string was expected as the TERMINATion message')

                        message = f' - {args[1]}'

                    if args[0] > 0:
                        print(f"{Fore.RED}Terminated with Error Code #{args[0]}{Style.RESET_ALL}{message}", end='')
                        self.InterpreterRunning = False
            ...
            case 'RESET':
                ...
                self._specialized_cache.cache[0] = 1

            case 'SUM':
                for arg in args:
                    if isinstance(arg, (str, char, bytes, bytearray)):
                        self.raise_error(TypeError, 'all SUM arguments must be numeric values')

                self._cache.set_cache(sum(args))

            case 'SUB':
                if len(args) != 2:
                    self.raise_error(SyntaxError, f"2 arguments were expected, but {len(args)} arguments were received")

                if isinstance(args[0], (str, char, bytes, bytearray)) or isinstance(args[1], (str, char, bytes, bytearray)):
                    self.raise_error('both SUB arguments must be numeric values')

                self._cache.set_cache(args[0] - args[1])
            ...
            case 'DIV':
                ...
                if isinstance(args[0], (str, char, bytes, bytearray)) or isinstance(args[1], (str, char, bytes, bytearray)):
                    self.raise_error('both DIV arguments must be numeric values')
            ...
            case 'PROD':
                ...
                if isinstance(args[0], (str, char, bytes, bytearray)) or isinstance(args[1], (str, char, bytes, bytearray)):
                    self.raise_error('both PROD arguments must be numeric values')
            ...
            case 'REM':
                ...
                if isinstance(args[0], (str, char, bytes, bytearray)) or isinstance(args[1], (str, char, bytes, bytearray)):
                    self.raise_error('both REM arguments must be numeric values')
            ...
            case 'ROUND':
                ...
                if isinstance(args[0], (str, char, bytes, bytearray, int, bool)):
                    self.raise_error('the number to round must be a float')

                if isinstance(args[1], (str, char, bytes, bytearray, float)):
                    self.raise_error('the amount of digits must be an integer')

                self._cache.set_cache(round(args[0], args[1]))

            case 'ABS':
                if len(args) != 1:
                    self.raise_error(SyntaxError, f"1 argument was expected, but {len(args)} arguments were received")

                if isinstance(args[0], (str, char, bytes, bytearray)):
                    self.raise_error('the ABS argument must be a numeric value')

                self._cache.set_cache(abs(args[0]))
            ...
            case 'FILE.EXISTS':
                ...
                if not isinstance(args[0], (str, char)):
                    self.raise_error('the filepath must be a string or a character')

                if os.path.exists(str(args[0])):
                    if os.path.isfile(str(args[0])):
                        self._specialized_cache.cache[0] = 1

                self._specialized_cache.cache[0] = 0
            ...
            case 'DIR.EXISTS':
                ...
                if not isinstance(args[0], (str, char)):
                    self.raise_error('the directory path must be a string or a character')
            ...
            case 'PATH.EXISTS':
                ...
                if not isinstance(args[0], (str, char)):
                    self.raise_error('the path must be a string or a character')

                self._specialized_cache.cache[0] = int(os.path.exists(str(args[0])))
